<div align="center">
  <h1>Hi, I'm Ghost 👋</h1>
  
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=22&duration=3000&pause=1000&color=3670A0&center=true&vCenter=true&width=500&lines=Systems+%26+Deep-Tech+Enthusiast;B.Tech+Computer+Science+Student;Building+From+First+Principles" alt="Typing SVG" />

  <p style="max-width: 600px; margin-top: 10px; color: #8b949e;">
    Building a rigorous, ground-up foundation in computer science, logical reasoning, and algorithmic thinking. Aspiring to engineer high-performance computing systems and deep technology infrastructure.
  </p>
</div>

---

## 🔬 Profile Summary

<table>
  <tr>
    <td width="50%" valign="top">
      <h3>🔥 Areas of Interest</h3>
      <ul>
        <li><strong>High-Performance Computing (HPC)</strong></li>
        <li>Computer Architecture & Hardware Design</li>
        <li>Parallel Programming & Compute Accelerators</li>
        <li>Systems Programming & Operating Systems</li>
        <li>Algorithmic Problem Solving</li>
      </ul>
    </td>
    <td width="50%" valign="top">
      <h3>⚡ Core Philosophy</h3>
      <ul>
        <li><strong>First Principles:</strong> Prioritizing deep conceptual mechanics over superficial usage.</li>
        <li><strong>Parallel Mastery:</strong> Integrating low-level concepts (CUDA/C++) early in development.</li>
        <li><strong>Pragmatic Debugging:</strong> Mastering technical depth through error isolation and analysis.</li>
      </ul>
    </td>
  </tr>
</table>

---

# 🛠 Technical Stack & Foundations

### 👨‍💻 Core Languages
<p align="left">
  <img src="https://img.shields.io/badge/C++-00599C?style=for-the-badge&logo=cplusplus&logoColor=white"/>
  <img src="https://img.shields.io/badge/Python-3670A0?style=for-the-badge&logo=python&logoColor=ffdd54"/>
  <img src="https://img.shields.io/badge/Java-ED8B00?style=for-the-badge&logo=openjdk&logoColor=white"/>
</p>

### ⚡ Parallel Computing & Hardware-Level Acceleration
<p align="left">
  <img src="https://img.shields.io/badge/CUDA-76B900?style=for-the-badge&logo=nvidia&logoColor=white"/>
  <img src="https://img.shields.io/badge/Computer_Architecture-111111?style=for-the-badge"/>
  <img src="https://img.shields.io/badge/HPC-005571?style=for-the-badge"/>
</p>

### 🧰 Workspace & Tools
<p align="left">
  <img src="https://img.shields.io/badge/VS_Code-0078D4?style=for-the-badge&logo=visualstudiocode"/>
  <img src="https://img.shields.io/badge/Linux_WSL-FCC624?style=for-the-badge&logo=linux&logoColor=black"/>
  <img src="https://img.shields.io/badge/Git-FF5733?style=for-the-badge&logo=git&logoColor=white"/>
  <img src="https://img.shields.io/badge/GitHub-000000?style=for-the-badge&logo=github"/>
</p>

---

# 📈 Milestones & Progress

<details>
  <summary><b>🎯 Current Academic Focus</b></summary>
  <br/>
  <ul>
    <li>Fourth-semester rigorous B.Tech coursework in Computer Science and Engineering.</li>
    <li>Deep-diving into Design and Analysis of Algorithms (DAA).</li>
    <li>Structuring technical roadmaps for graduate-level assessments and international research profile milestones.</li>
  </ul>
</details>

<br/>

<div align="center">
  <h3>📊 Activity Metrics</h3>
  
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/stats?username=GhostR41&theme=tokyonight" width="48%" />
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/repos-per-language?username=GhostR41&theme=tokyonight" width="48%" />
  
  <br/>
  
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=GhostR41&theme=tokyonight" width="97%" />
</div>

---

# 🌐 Connectivity

<div align="left">
  <a href="[mailto:your.email@example.com](https://guthib.com/)">
    <img src="https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white"/>
  </a>
  <a href="https://www.linkedin.com/in/pranjaysh141/">
    <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white"/>
  </a>
</div>
